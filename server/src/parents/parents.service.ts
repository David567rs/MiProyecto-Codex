...
export class ParentsService {
        ...
        async findAllUnassignedWithChildren(): Promise<any> {
                try {
                        ...
                        console.error(
                                'Error finding unassigned parents with children:',
                                error,
                        );
                        throw new InternalServerErrorException(
                                'Could not retrieve unassigned parents with children',
                        );
                }
        }
        ...
        async findNurseByEmail(email: string): Promise<string> {
                try {
                        ...
                        if (!nurse) {
                                console.error(
                                        `Nurse with email ${email} not found`,
                                );
                        ...
                        throw new InternalServerErrorException(
                                'Could not retrieve nurse by email',
                        );
                }
        }
        ...
        async assignToNurse(
                ...
                try {
                        ...
                        const parentUpdateResult = await this.userModel
                        ...
                        const childrenUpdateResult = await this.childrenModel
                                .updateMany(
                                        { parentId: parentId },
                                        { assignedNurse: nurseId },
                                )
                        ...
                        console.log(
                                ...
                                childrenUpdateResult,
                        );
                } catch (error) {
                        console.error(
                                'Error assigning nurse to parent and children:',
                                error,
                        );
                        throw new InternalServerErrorException(
                                'Could not assign nurse to parent and children',
                        );
                }
        }
        ...
        async findParentDetails(parentId: string): Promise<any> {
                try {
                        ...
                        throw new InternalServerErrorException(
                                'Could not retrieve parent details',
                        );
                }
        }
        ...
        private calculateExpectedVaccineDate(
                birthDate: Date,
                months: number,
        ): Date {
        ...
        public async getVaccinationData(childId: string) {
                ...
                const parent = await this.userModel
                        .findById(child.parentId)
                        .exec();
                if (!parent) {
                        throw new NotFoundException('Parent not found.');
                }

                const vaccineMonths = await this.vaccineMonthModel
                        .find()
                        .lean()
                        .exec();
                const notifications = [];
                ...
                const allVaccines = await this.vaccineModel
                        .find()
                        .lean()
                        .exec();
                const vaccineMap = allVaccines.reduce(
                        (acc, vaccine) => {
                ...
                for (const vaccineMonth of vaccineMonths) {
                        const expectedVaccineDate =
                                this.calculateExpectedVaccineDate(
                                        birthDate,
                                        vaccineMonth.month,
                                );
                        const currentDate = new Date();

                        const missingVaccines = vaccineMonth.vaccines.filter(
                                (vaccineId) =>
                                        !childVaccines.includes(
                                                vaccineId.toString(),
                                        ),
                        );
                                        ...
                                        notifications.push(
                                                ...missingVaccines.map(
                                                        (vaccineId) => ({
                                                                ...
                                                                expectedVaccineDate,
                                                                delayDays: this.calculateDaysDifference(
                                                                        expectedVaccineDate,
                                                                        currentDate,
                                                                ),
                                                        }),
        ...
        public async getVaccinationDataDetails(childId: string) {
                try {
                        ...
                        for (const vaccineMonth of vaccineMonths) {
                                try {
                                        ...
                                        const missingVaccines =
                                                vaccineMonth.vaccines.filter(
                                                        (vaccineId) =>
                                                                !appliedVaccines.some(
                                                                        (
                                                                                applied,
                                                                        ) =>
                                                                                applied.vaccineId ===
                                        ...
                                        const appliedInMonth =
                                                appliedVaccines.filter(
                                                        (applied) =>
                                                                applied.month ===
                                                                vaccineMonth.month,
                                                );
                                        const confirmedInMonth =
                                                confirmationVaccines.filter(
                                                        (confirmed) =>
                                                                confirmed.month ===
                                                                vaccineMonth.month,
                                                );
                                        ...
                                        for (const applied of appliedInMonth) {
                                                try {
                                                        ...
                                                        if (vaccine) {
                                                                ...
                                                                console.warn(
                                                                        `Vaccine with ID ${applied.vaccineId} not found in vaccineMap`,
                                                                );
                                                        }
                                                } catch (error) {
                                                        console.error(
                                                                `Error processing applied vaccine with ID ${applied.vaccineId}:`,
                                                                error,
                                                        );
                                                }
                                        }
                                        ...
                                        for (const confirmed of confirmedInMonth) {
                                                try {
                                                        ...
                                                        if (vaccine) {
                                                                ...
                                                                console.warn(
                                                                        `Vaccine with ID ${confirmed.vaccineId} not found in vaccineMap`,
                                                                );
                                                        }
                                                } catch (error) {
                                                        console.error(
                                                                `Error processing confirmed vaccine with ID ${confirmed.vaccineId}:`,
                                                                error,
                                                        );
                                                }
                                        }
                                        ...
                                        console.error(
                                                `Error processing vaccineMonth for month ${vaccineMonth.month}:`,
                                                error,
                                        );
                                }
                        }
                        ...
                        console.error(
                                'Error in getVaccinationDataDetails function:',
                                error,
                        );
                        throw error;
                }
        ...
        public async applyVaccine(
                ...
                const alreadyApplied = child.appliedVaccines?.some(
                        (applied) =>
                                applied.vaccineId === vaccineId &&
                                applied.month === month,
                );

                if (alreadyApplied) {
                        throw new Error(
                                'Vaccine already applied for this month.',
                        );
                }
        ...
        public async confirmationVaccine(
                childId: string,
                month: number,
                vaccineId: string,
        ) {
        ...
        public async getConfirmedVaccines(childId: string): Promise<any> {
                try {
                        ...
                        console.error(
                                'Error getting confirmed vaccines:',
                                error,
                        );
                        throw new InternalServerErrorException(
                                'Could not retrieve confirmed vaccines',
                        );
                }
        }
}
